...
export function renderSuiviPage(token: string, nomProjet: string): string {
  return `${renderHead(
    `Suivi de commande — ${nomProjet}`,
    `Suivez votre commande en temps réel sur ${nomProjet}.`,
    nomProjet
  )}
<body class="font-sans bg-gray-50 min-h-screen">
  ...
  <main class="max-w-lg mx-auto px-4 py-8">
    ...
    <!-- Aide -->
    <div class="mt-6 bg-white border border-gray-100 rounded-xl p-4 text-center">
  ...
  <script>
    ...
    async function chargerSuivi() {
      try {
            ...
            return \`
              <div class="flex items-start gap-3">
                ...
                <div>
                  ...
                  \${h.commentaire ? \`<div class="text-xs text-gray-600 mt-0.5">\${h.commentaire}</div>\` : ''}
                </div>
              </div>
            \`;
          }).join('');
        ...
        console.error(e);
        document.getElementById('suivi-content').innerHTML = \`
          <div class="bg-yellow-50 border border-yellow-100 rounded-xl p-6 text-center">
