...
export function genererMessageCommande(commande: Commande, tenant: Tenant): string {
  ...
  const mapsUrl = commande.client_latitude && commande.client_longitude
    ? `https://maps.google.com/?q=${commande.client_latitude},${commande.client_longitude}`
    : ''
  const wazeUrl = commande.client_latitude && commande.client_longitude
  ...
  message += `\n*Commande :*\n${lignesItems}\n\n`
  message += `*Sous-total :* ${formatMontant(commande.montant_total - commande.frais_livraison)}\n`
